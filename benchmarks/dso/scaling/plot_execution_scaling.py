#!/usr/bin/env python
"""Plot DSO execution-scaling results."""
...
def plot(rows, figure_dir: Path) -> list[Path]:
    ...
    ax.set_title("DSO execution scaling", loc="left")
    ...
    paths = [
        figure_dir / "dso_execution_scaling_matched_range.pdf",
        figure_dir / "dso_execution_scaling_matched_range.png",
    ]
    for p in paths:
...
def main(argv=None):
    ...
    for p in paths:
        print(f"[dso_scaling_plot] wrote {p}")
