...
PowerZooJax 中的集成电力系统模型跨越五个相互关联的物理层级：

![PowerZooJax 物理模型概览图](../../assets/images/physics/fig1.png){ width="100%" }

*图 1. PowerZooJax 中各物理建模层级及其交互关系总览。*

## 如何阅读图 1

图 1 将 PowerZooJax 的物理建模划分为 3 个**系统模型**（输电、配电、微电网）、2 个**资源层**（发电侧、终端 DER），以及 1 个**协调层**（市场）。能量流大体从左到右（发电 → 负荷），信息与协调信号（例如价格）则跨层传播，影响各层决策。
